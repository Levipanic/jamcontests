...
func (a *App) adminVotesPage(c *gin.Context) {
	...
	rows, err := a.pool.Query(c.Request.Context(), `
		SELECT vote.id, user_account.username, nomination.title, product.title, team.name,
		       vote.invalidated_at IS NOT NULL, vote.updated_at
	...
	for rows.Next() {
		...
		if err = rows.Scan(&row.ID, &row.Username, &row.NominationTitle, &row.ProductTitle, &row.TeamName, &row.Invalidated, &row.UpdatedAt); err != nil {
...
func (a *App) adminBumpsPage(c *gin.Context) {
	...
	for rows.Next() {
		...
		if err = rows.Scan(&row.ID, &row.Username, &row.ProductTitle, &row.TeamName, &row.BumpCount, &row.InvalidatedCount, &row.LastBumpedAt); err != nil {
...
func (a *App) adminMutateVote(c *gin.Context, invalidate bool) {
	...
	err = tx.QueryRow(ctx, `SELECT user_id,nomination_id,product_id,invalidated_at FROM nomination_votes WHERE id=$1 AND jam_id=$2 FOR UPDATE`, recordID, jamID).Scan(&userID, &nominationID, &productID, &invalidatedAt)
	...
	if invalidate {
		_, err = tx.Exec(ctx, `UPDATE nomination_votes SET invalidated_at=clock_timestamp(), invalidated_by=$2 WHERE id=$1`, recordID, CurrentUser(c).ID)
	} else {
		_, err = tx.Exec(ctx, `UPDATE nomination_votes SET invalidated_at=NULL, invalidated_by=NULL WHERE id=$1`, recordID)
	}
	if err != nil {
...
func (a *App) adminMutateBump(c *gin.Context, invalidate bool) {
	...
	err = tx.QueryRow(ctx, `SELECT user_id,product_id,bump_count,invalidated_count FROM product_bumps WHERE id=$1 AND jam_id=$2 FOR UPDATE`, recordID, jamID).Scan(&userID, &productID, &count, &invalidated)
	if errors.Is(err, pgx.ErrNoRows) {
		c.AbortWithStatus(http.StatusNotFound)
		return
	}
	...
	if invalidate {
		_, err = tx.Exec(ctx, `UPDATE product_bumps SET invalidated_count=bump_count, invalidated_at=clock_timestamp(), invalidated_by=$2 WHERE id=$1`, recordID, CurrentUser(c).ID)
	} else {
		_, err = tx.Exec(ctx, `UPDATE product_bumps SET invalidated_count=0, invalidated_at=NULL, invalidated_by=NULL WHERE id=$1`, recordID)
	}
	if err != nil {
