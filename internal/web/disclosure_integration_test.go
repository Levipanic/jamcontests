...
func TestPublicDisclosureMatrixAcrossVisibilityAndStages(t *testing.T) {
	...
	for _, test := range stages {
		t.Run(string(test.stage), func(t *testing.T) {
			...
			assertDisclosureResponse(t, router, "/jams/"+formatID(fixture.jamID)+"/products", test.products, "Product B")
			assertDisclosureResponse(t, router, "/products/"+formatID(fixture.productBID), test.products, "Product B")
			nominations := disclosureRequest(router, "/jams/"+formatID(fixture.jamID)+"/nominations")
			...
			assertDisclosureResponse(t, router, votingCountsPath(fixture.jamID), test.votes, `"counts"`)
			assertDisclosureResponse(t, router, "/api/products/"+formatID(fixture.productBID)+"/bumps", test.bumps, `"count"`)
			home := disclosureRequest(router, "/jams/"+formatID(fixture.jamID))
			if strings.Contains(home.Body.String(), themeSentinel) != test.themes {
				t.Fatalf("theme disclosure mismatch: %s", home.Body.String())
			}
			team := disclosureRequest(router, "/teams/"+formatID(productTeamID))
	...
	for _, path := range []string{
		"/jams/" + formatID(fixture.jamID),
		"/jams/" + formatID(fixture.jamID) + "/products",
		"/products/" + formatID(fixture.productBID),
		"/jams/" + formatID(fixture.jamID) + "/nominations",
		votingCountsPath(fixture.jamID),
		"/api/products/" + formatID(fixture.productBID) + "/bumps",
	} {
